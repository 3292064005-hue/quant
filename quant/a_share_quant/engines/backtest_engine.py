...
from a_share_quant.core.metrics import PerformanceMetrics, RelativePerformanceMetrics, compute_metrics, compute_relative_metrics
from a_share_quant.core.utils import new_id
from a_share_quant.core.valuation import PortfolioValuator, ValuationResult
from a_share_quant.domain.models import BacktestResult, BacktestRunStatus, Bar, DataLineage, RiskResult, RunArtifacts, Security, TargetPosition, TradingCalendarEntry
...
class RunEventCollector:
    ...
    def handle(self, event: Event) -> None:
        self.events.append({"type": event.event_type, "payload": event.payload})
...
class RebalancePlanner:
    ...
    def plan(self, strategy, frame: TradeDayFrame, account, positions: dict[str, object]) -> RebalanceDecision:
        required_history = strategy.required_history_bars()
        has_strategy_input = any(len(bars) >= required_history for bars in frame.active_history.values())
        should_rebalance = has_strategy_input and strategy.should_rebalance(self.eligible_trade_index)
        targets: list[TargetPosition] = []
        if has_strategy_input:
            if should_rebalance:
                targets = strategy.generate_targets(frame.active_history, frame.trade_date, frame.active_securities)
...
class ExecutionCoordinator:
    ...
    def execute(self, orders: list, frame: TradeDayFrame, positions: dict[str, object], account, target_weights: dict[str, float]) -> CoordinatedExecution:
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.event_bus.subscribe("DAY_CLOSED", self.event_collector.handle)
    ...
    def _run_over_trade_days(
        ...
        history_window = max(strategy.required_history_bars(), 1)
        ...
        planner = RebalancePlanner(self.portfolio_engine)
        ...
        try:
            for current_date, day_bars in day_batches:
                ...
                self.event_bus.publish(Event("DAY_CLOSED", {"trade_date": current_date.isoformat(), "equity": eod_valuation.account.total_assets, "run_id": run_id}))
