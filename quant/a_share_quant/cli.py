...
_PACKAGED_SAMPLE_SENTINEL = "__packaged_sample_data__"
...
def _require_distribution_capability(config, *, capability: str, command_name: str) -> None:
    """校验 distribution profile 是否允许给定运行面。"""
    capabilities = config.distribution_capabilities()
    if not bool(capabilities.get(capability)):
        raise SystemExit(
            f"当前命令 {command_name} 不支持 app.distribution_profile={config.app.distribution_profile}；缺少能力 {capability}。"
        )
...
def _validate_operator_cli_config(config_path: str, *, broker_client_factory: str | None) -> None:
    ...
    _require_distribution_capability(config, capability="supports_operator_runtime", command_name="operator CLI")
    ...
    *,
    ...
    results.append(
        check_broker_runtime(
            ...
            acceptance_manifest_path=config.broker.acceptance_manifest_path,
            execute_acceptance_suite=config.broker.execute_acceptance_suite,
            required_readiness_level=config.broker.required_readiness_level,
...
def _load_ui_operations_snapshot(config_path: str, *, runtime_results: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    返回值保留原始 snapshot 作为兼容层，同时以 ``ui_*`` 投影作为桌面层正式契约。
    """
    ...
    with bootstrap_data_context(config_path) as context:
        ...
        workflow_registry = capabilities.require_workflow_registry() if capabilities.registries.workflow_registry is not None else None
        query_service = RunQueryService(
            backtest_run_repository=capabilities.persistence.backtest_run_repository,
            order_repository=capabilities.persistence.order_repository,
            audit_repository=capabilities.persistence.audit_repository,
            data_import_repository=capabilities.persistence.data_import_repository,
            research_run_repository=capabilities.persistence.research_run_repository,
            execution_session_repository=capabilities.persistence.execution_session_repository,
            account_repository=capabilities.persistence.account_repository,
            runtime_event_repository=getattr(context, "runtime_event_repository", None),
        )
        plugin_manager: PluginManager | None = capabilities.registries.plugin_manager
        provider_details = (
            [entry.to_summary() for entry in context.require_provider_registry().list_entries()]
            if capabilities.registries.provider_registry is not None
            else []
        )
        ...
        snapshot["config_runtime"] = {
            "runtime_mode": capabilities.config.app.runtime_mode,
            "broker_provider": capabilities.config.broker.provider,
        }
        ...
        snapshot["registered_components"] = (
            context.require_component_registry().list_component_summaries() if capabilities.registries.component_registry is not None else []
        )
        snapshot.update(
...
def _require_ui_official_runtime_scope(config) -> None:
    """限制桌面只读面板仅在官方支持的 workstation + research_backtest + mock 组合下启动。"""
    if config.app.distribution_profile != "workstation":
        raise SystemExit(
            f"桌面只读运营面板仅支持 app.distribution_profile=workstation；收到 {config.app.distribution_profile}。"
            "core/production 发行形态请使用 headless CLI 或 operator_snapshot。"
        )
...
def _load_operator_snapshot(config_path: str, *, broker_client_factory: str | None = None) -> dict[str, Any]:
    ...
    runtime_results = _build_runtime_results(config, broker_client_factory=broker_client_factory, config_path=config_path)
    with bootstrap_operator_context(config_path, broker_client_factory=broker_client_factory) as context:
        capabilities = context.operator_capabilities()
        query_service = RunQueryService(
            backtest_run_repository=capabilities.persistence.backtest_run_repository,
            order_repository=capabilities.persistence.order_repository,
            audit_repository=capabilities.persistence.audit_repository,
        ...
        broker = capabilities.require_broker()
        return query_service.build_operator_snapshot(
            broker=broker,
            runtime_mode=capabilities.config.app.runtime_mode,
            broker_provider=capabilities.config.broker.provider,
            default_account_id=capabilities.config.broker.account_id or None,
            allowed_account_ids=capabilities.config.broker.allowed_account_ids,
            event_source_mode=capabilities.config.broker.event_source_mode,
            supervisor_config={
                "scan_interval_seconds": capabilities.config.operator.supervisor_scan_interval_seconds,
                "lease_seconds": capabilities.config.operator.supervisor_lease_seconds,
                "heartbeat_interval_seconds": capabilities.config.operator.supervisor_heartbeat_interval_seconds,
                "idle_timeout_seconds": capabilities.config.operator.supervisor_idle_timeout_seconds,
                "max_sessions_per_pass": capabilities.config.operator.supervisor_max_sessions_per_pass,
            },
...
def _require_research_backtest_mode(config_path: str) -> None:
    config = ConfigLoader.load(config_path)
    _require_distribution_capability(config, capability="supports_research_workflow", command_name="research/backtest CLI")
    if config.app.runtime_mode != "research_backtest":
        raise SystemExit(
...
def _run_default_backtest(
    ...
    *,
    ...
    with bootstrap(config_path) as context:
        ...
        if import_csv_path:
            if import_csv_path == _PACKAGED_SAMPLE_SENTINEL:
                sample_resource = resources.files("a_share_quant.sample_data").joinpath("daily_bars.csv")
                with resources.as_file(sample_resource) as sample_path:
                    data_service.import_csv(str(sample_path), encoding=context.config.data.default_csv_encoding)
            else:
...
def main_check_runtime(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_runtime import main_check_runtime as _impl

    return _impl(argv)


def main_daily_run(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_backtest import main_daily_run as _delegate

    return _delegate(argv)


def main_generate_report(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_report import main_generate_report as _delegate

    return _delegate(argv)


def main_init_db(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_data import main_init_db as _delegate
...
def main_operator_snapshot(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_snapshot as _impl

    return _impl(argv)


def main_operator_submit_order(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_submit_order as _impl

    return _impl(argv)


def main_operator_submit_signal(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_submit_signal as _impl

    return _impl(argv)


def main_operator_reconcile_session(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_reconcile_session as _impl

    return _impl(argv)


def main_operator_sync_session(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_sync_session as _impl

    return _impl(argv)


def main_operator_run_supervisor(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_operator import main_operator_run_supervisor as _impl

    return _impl(argv)


def main_launch_ui(argv: list[str] | None = None) -> int:
    from a_share_quant.cli_runtime import main_launch_ui as _impl
