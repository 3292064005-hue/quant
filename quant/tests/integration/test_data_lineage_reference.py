...
def test_backtest_lineage_ignores_latest_failed_import_run(temp_config_dir: Path) -> None:
    ...
    with bootstrap(str(app_path)) as context:
        ...
        completed_import_run_id = data_service.last_import_run_id
    ...
    with bootstrap(str(app_path)) as context:
        ...
        result = context.require_backtest_service().run(strategy, entrypoint="tests.integration.failed_import_lineage")
