...
def test_tushare_adapter_maps_bundle() -> None:
    ...
    assert bundle.degradation_flags == []
...
def test_tushare_adapter_preserves_successful_status_frames_when_one_status_fails() -> None:
    ...
    bundle = adapter.fetch_bundle(start_date="20260101", end_date="20260106")
    assert len(bundle.securities) == 2
    assert "stock_basic_status_missing:D" in bundle.degradation_flags
    assert any("list_status=D" in warning for warning in bundle.warnings)


class _LimitMissingTushareClient(_FakeTushareClient):
    def stk_limit(self, **kwargs):
        raise RuntimeError("no permission")


def test_tushare_adapter_marks_limit_data_degradation() -> None:
    adapter = TushareDataAdapter(token="dummy", client=_LimitMissingTushareClient())
    bundle = adapter.fetch_bundle(start_date="20260101", end_date="20260106")
    assert "stk_limit_unavailable" in bundle.degradation_flags
    assert any("stk_limit" in warning for warning in bundle.warnings)
