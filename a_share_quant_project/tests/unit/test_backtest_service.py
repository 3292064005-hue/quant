...
class _EngineStub:
    def run(self, strategy, bars_by_symbol, securities, config_snapshot, benchmark_symbol):
...
class _RunRepositoryStub:
    def __init__(self) -> None:
        self.calls: list[tuple[str, BacktestRunStatus, str | None, str | None]] = []

    def finish_run(self, run_id: str, status: BacktestRunStatus, error_message: str | None = None, report_path: str | None = None) -> None:
        self.calls.append((run_id, status, error_message, report_path))
...
def test_backtest_service_marks_success_once(tmp_path: Path) -> None:
    ...
    assert run_repository.calls == [
        ("run_demo", BacktestRunStatus.COMPLETED, None, str(report_path))
    ]
